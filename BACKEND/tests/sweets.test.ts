...
//       .send({ name: "Candy", price: 1, category: "Hard", quantity: 10 });
...
// //   it("GET /api/sweets/search?q=Candy should return matching sweets", async () => {
// //     const res = await request(app).get("/api/sweets/search?q=Candy");
...
// //   it("GET /api/sweets/search with no query should return empty array", async () => {
...
//   it("PUT /api/sweets/:id should return 404 if not found", async () => {
//     const res = await request(app)
//       .put("/api/sweets/64f000000000000000000000")
//       .set("Authorization", `Bearer ${adminToken}`)
//       .send({ price: 5 });
